...
const displayCountryDetail = async () => {
    ...
    try {
        ...
        const borderCountriesHTML = borderCountries.length > 0 
            ? borderCountries.map(country => `<span class="border-country">${country}</span>`).join('')
        ...
        countryDetailContainer.innerHTML = `
            ...
            <div class="country-info">
                ...
                <div class="detail-grid">
                    ...
                    <div class="right-col">
                        <p><strong>Top Level Domain: </strong>${selectedCountry.tld?.[0] || 'N/A'}</p>
                        <p><strong>Currencies: </strong>${Object.values(selectedCountry.currencies || {}).map(c => c.name).join(', ') || 'N/A'}</p>
        ...
        document.querySelectorAll('.border-country').forEach((btn, index) => {
            if (borderCountries[index] && btn.textContent !== 'None') {
                btn.onclick = () => {
                    const borderCountry = allCountries.find(c => c.name.common === btn.textContent);
                    if (borderCountry) {
